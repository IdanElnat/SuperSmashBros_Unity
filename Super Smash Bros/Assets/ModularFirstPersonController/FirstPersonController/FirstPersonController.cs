...
public class FirstPersonController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown && !rd.IsRagdoll())
            {
    ...
    private void CheckGround()
    {
        ...
        float distance = .75f;
        if (this.GetComponent<Survivability>().IsEnlarged()) {
           
            distance *=2.7f;
        }
        Debug.DrawRay(origin, direction * distance, Color.red);
  
        if ((Physics.Raycast(origin, direction, out RaycastHit hit, distance) && timeSinceJump > 0.5f) || this.GetComponent<PlayerController>().IsLedged())
        {
    ...
    private void Jump()
    {
        ...
        if (isGrounded|| timeSinceJump <= 0f)
        {
